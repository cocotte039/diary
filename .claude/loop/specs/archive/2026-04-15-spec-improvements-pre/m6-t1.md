...
5. `BookshelfPage.module.css` に `.calendarOverlay` / `.calendarPanel` / `.calendarClose` 追加、`.calendarToggle` 削除
6. テスト 3 件追加

## BookshelfPage.tsx 変更点

```tsx
// calendarToggle ボタンブロック（L137-145 相当）を削除
// {showCalendar && <Calendar />} を以下に置換:
...
- `Calendar` 内で日付選択時の navigate 後、BookshelfPage は `/book/:id/:page` に遷移し unmount → overlay 自動消滅
- z-index: overlay 100 / menu 50 で重ならない
- `within` は testing-library の import を確認
