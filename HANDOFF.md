...
## 10. M7 実機検証チェックリスト（ヘッダー整合・日付アイコン）

M7 で追加したレイアウト変更は jsdom では完全に検証できないため、
必ず実機で以下を確認すること。

- [ ] **ヘッダーと本文 1 行目が重ならない**: EditorPage を開き、ヘッダー直下に
      本文 1 行目（の罫線ではなく文字）が食い込まないこと。`--header-height = 57.6px`。
- [ ] **罫線と本文ベースラインが揃う**: 本文の各行が罫線の直上にぴったり乗る。
      ズレる場合は `src/styles/global.css` の `--line-height` / `--font-size` を微調整。
- [ ] **iOS Safari の safe-area-inset-top**: ノッチ付き端末で PWA として開き、
      ヘッダーがステータスバーに隠れないこと。`.header` の `height` は
      `calc(var(--header-height) + env(safe-area-inset-top, 0px))` で計算されるが、
      実機挙動は jsdom では確認できない。
- [ ] **日付アイコンのタップ領域**: ヘッダー右のアイコンをタップするとカーソル位置に
      `YYYY年M月D日(曜)\n` が挿入される。16x16 のアイコンだが 44x44 の hit area。
- [ ] **旧 /write / /read URL の挙動**: ブックマークしていたら `/` (本棚) か `/book/:id/:page`
      に redirect されること。
