...
- IndexedDB によるローカル永続化（ページ単位 savePage、2 秒 debounce、ページめくり/「本棚」離脱/`popstate`/アプリ背面化（`visibilitychange`・`pagehide`）時に flush して末尾入力をロストしない。読込内容はベースライン化し、読むだけの離脱では無変更保存・無駄同期をしない）
